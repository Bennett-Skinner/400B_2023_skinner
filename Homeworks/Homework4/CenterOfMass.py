...
# Solutions: B. Skinner, G. Besla, R. Li, H. Foote
...
if __name__ == '__main__' : 
    ...
    # Create center of mass objects for MW, M31 and M33
    ...
    # Answer the questions (print statement texts explain what each calculation is)
    print("Q2")
    print("MW-M31 Distance: " + str(np.round(np.sqrt(np.sum((MW_COM_p-M31_COM_p)**2)),3))
          + ", MW-M31 Mutual Velocity: " + str(np.round(np.sqrt(np.sum((MW_COM_v-M31_COM_v)**2)),3)))
    print("This is in line with the MW-M31 distance and mutual velocity given in lecture.")
    print("Q3")
    print("M31-M33 Distance " + str(np.round(np.sqrt(np.sum((M33_COM_p-M31_COM_p)**2)),3))
          + ", M31-M33 Mutual Velocity: " + str(np.round(np.sqrt(np.sum((M33_COM_v-M31_COM_v)**2)),3)))
    print("We find M31 is much closer to M33 then MW, as it should be because M33 is a M31 satellite.")
    print("Q4")
    print("The tidal interactions between MW and Andromeda are pulling stars at the edge of the disk "+
          "outward, towards the other galaxy. If we included these flung-out stars, our COMs would be "+
          "biased towards the position of the other galaxy. Hence, we exclude the outermost stars.")    
